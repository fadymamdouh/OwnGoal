...
| L34 | VAR reviewing an Offside call | **Closed — OFFSIDE calls are NOT reviewable by VAR.** The rulebook lists offside alongside goal and penalty as things VAR can review, but the engine has never offered a react_var phase after OFFSIDE stops an attack. After investigation: implementing it requires a new pending phase (react_var_offside), and the attacker — not the defender — would be the one calling heads/tails, since they were ruled offside. The asymmetry (defender plays VAR *to confirm* an offside that already stopped the attack) is confusing enough that the cleaner design is to leave OFFSIDE non-reviewable. The rulebook line about reviewing offside is removed. **OFFSIDE counters PASS / DRIBBLE / ASSIST / SHOT_GOAL / GOAL as a hard stop — no VAR appeal.** |
